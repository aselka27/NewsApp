...
struct NetworkManager {
    ...
    func decodeData<T: Codable> (ofType: T.Type, urlRequest: URLRequest, response: @escaping ([T])->Void) {
    ...
    func decodeDataP<T: Codable> (ofType: T.Type, urlRequest: URLRequest, response: @escaping (T)->Void) {
        getRequest(urlRequest: urlRequest) { result in
            switch result {
            case .success(let data):
                do {
